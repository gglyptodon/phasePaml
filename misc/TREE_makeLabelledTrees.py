...
def showTreeWithPictures(tree = None, alignment=None, branchLengths=True, bootstrapSupport=True, tolabel=None,showZScores=False,showLogs=False ):
    print(PICS)
    ts = TreeStyle()
    ts.title.add_face(TextFace(FILE.split(os.sep)[-1]),column = 0)
    ...
    if not alignment:
        ...
        #
        HEIGHT=None
        for node in t.iter_descendants(strategy="levelorder"):
            print(node.node_id)
            #node.unroot()
            #node.set_style(nsFG)
            ...
            if (node.name.split("_")[0]+".png" in PICS):
                ...
                node.add_face(ImgFace(PICDIR+os.sep+node.name.split("_")[0]+".png", height=HEIGHT), column=1, position="aligned")
            #non GRZM identifier
            elif (node.name+".png" in PICS):
                print(node.name+".png")
                node.add_face(ImgFace(PICDIR+os.sep+node.name+".png", height=HEIGHT), column=1, position="aligned")
        ...
        if L:
            out+="_L"
        t.render(out+"_tree_lab.pdf",tree_style=ts)
        t.render(out+"_tree_lab.png",tree_style=ts)
        if INTERACTIVE:
...
def labelForPamlRegex(unlabelledTreeAsString, regex, tree,bootstrapSupport = None, branchLengths = None, tolabel=None,showZScores=None,showLogs=None):
    ts = TreeStyle()
    ts.title.add_face(TextFace(FILE.split(os.sep)[-1]),column = 0)
    ...
    for node in t.get_descendants():
        node.add_face(TextFace(node.node_id), column=0)
        print(node.name.split("_")[0], PICS)
        if (node.name.split("_")[0]+".png" in PICS):
                print("MATCHED,",node.name.split("_")[0]+".png")
                node.add_face(ImgFace(PICDIR+os.sep+node.name.split("_")[0]+".png", height=HEIGHT), column=1, position="aligned")
    ...
    if L:
        out+="_L"
    t.render(out+"_tree_n.pdf",tree_style=ts)
    t.render(out+"_tree_n.png",tree_style=ts)
    if INTERACTIVE:
        t.show()
    return(outfiles)
...
def main():
    ...
    ###################################
    try:
        opts, args = getopt.gnu_getopt(sys.argv[1:], "t:T:a:n:r:blZLi", ["tree=","bootstrapTree=""alignment=","node=","regex=","showBootstrapSupport","ShowBranchLengths","help","zscore","log","interactive"])
    ...
    if regex:
        labelForPamlRegex(tree, regex, FILE,bootstrapSupport = showBootstrapSupport, branchLengths = showBranchLengths, tolabel=tolabel,showZScores=showZScores,showLogs=showLogs)
